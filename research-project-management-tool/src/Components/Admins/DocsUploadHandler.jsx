...
const DocsUploadHandler = () => {
  ...
  const handleOnSubmit = async (event) => {
    ...
    try {
      const { document_title, note, submission_type } = state;
      if (document_title.trim() !== '' && note.trim() !== '' && submission_type.trim() !== '') {
        if (file) {
          const formData = new FormData();
          formData.append('file', file);
          formData.append('document_name', document_title);
          console.log("YYY", document_title)
    ...
    <ThemeProvider breakpoints={['xxxl', 'xl', 'lg', 'md', 'sm', 'xs', 'xxs']}>
      ...
      <Container>
        ...
        <div Style={{ marginTop: '50px' }} className='list-title'>
          <hr />
          <center>
            <h2>Upload Document / Presentation Template</h2>
          </center>
          <hr />
        </div>
        <br />  <br />  <br />
...
export default DocsUploadHandler;
